...
class GoodController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $good->price        = $request['price'];
            $good->active       = $request['active'];
            if (strrpos($request['picture'], 'http://localhost:8000/img/') === false) {
                $this->validate($request, [
                    'picture'       => 'image64:jpeg,jpg,png'
                ]);
                $good->picture      = asset('img/'.$pictureName);
                Image::make($request['picture'])->encode('jpg')->resize(800, 800)->save(public_path('img/').$pictureName);
            }
            // if success
            if ($good->save()) {
                // return as JSON
