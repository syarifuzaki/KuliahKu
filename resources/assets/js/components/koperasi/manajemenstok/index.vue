            ...
            <div class="col-xs-12">
                ...
                <div class="card">
                    <ol class="breadcrumb breadcrumb-col-red">
                        ...
                        <li class="active"><i class="material-icons">list</i> Manajemen Stok</li>
                    </ol>
                </div>
                <!-- Konten utama -->
                <div class="card">
                                ...
                                <table class="table table-striped table-bordered table-hover" v-else>
                                    ...
                                    <tbody>
                                        <tr v-for="(row, index) in rows.data"  :key="row.id">
                                            ...
                                            <td>{{ row.user.name }}</td>
                                            <td v-html="labelType(row.value)"></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">{{ modalTitle }} Manajemen Stok</h4>
                    </div>
                    <form class="form-horizontal add" @submit.prevent="formAction(state.id)">
                        <div v-if="modalTitle == 'Tambah'">
                            <div class="modal-body">
                                ...
                                <div class="row clearfix">
                                    <div class="col-xs-4 form-control-label">
                                        <label for="type">Jenis Stok *</label>
                                    </div>
                                    <div class="col-xs-8">
                                        <div class="form-group">
                                            <input type="radio" id="+" value="+" class="with-gap radio-col-lime" v-model="state.type" />
                                            <label for="+">Menambah</label>
                                            <input type="radio" id="-" value="-" class="with-gap radio-col-brown" v-model="state.type" />
                                            <label for="-">Mengurangi</label>
                                        </div>
                        ...
                        <div v-if="modalTitle == 'Hapus'">
                            <div class="modal-body">
                                Apakah anda yakin ingin menghapus stok dengan <b>ID {{ state.id }}</b> ini?
                            </div>
                            <div class="modal-footer">
...
export default {
    data () {
        return {
            api: {
                index: 'api/koperasi/stok',
                lastid: 'api/koperasi/stok/getLastID',
                goods: 'api/koperasi/barang/get',
                userid: 'api/koperasi/getUserID',
            },
            color: 'bg-red',
            rows: {
                total: -1
    ...
    mounted () {
        ...
        this.getUserID(this.api.userid)
    },
    ...
    methods: {
        ...
        showDetail(id) {
            ...
            .then(response => {
                this.state.id               = response.data.id
                this.state.good_id          = response.data.good_id
                this.state.user_id          = response.data.user_id
